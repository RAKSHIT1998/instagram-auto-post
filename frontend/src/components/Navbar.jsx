...
export default function Navbar({ user, onRegisterDemo, loading, onLogout, onOpenOnboarding }) {
  return (
    <motion.header
      ...
      className="sticky top-0 z-20 backdrop-blur bg-bg/80 border-b border-white/10 px-4 md:px-8 py-4 flex items-center justify-between gap-4"
    >
      <div className="min-w-0">
        <p className="text-muted text-sm">AI Social SaaS</p>
        <h2 className="text-lg font-semibold truncate">Premium Growth Engine</h2>
        <p className="text-[11px] text-muted/80 mt-0.5 hidden xl:block truncate">Shortcuts: ⌘/Ctrl+K Commands • G Generate • D Dashboard • S Scheduled • A Analytics • I Integrations</p>
      </div>
      ...
      <div className="flex items-center gap-3 flex-shrink-0">
        {user ? (
          <>
            ...
            <div className="spotlight-card glass-card px-4 py-2 text-sm hidden md:flex items-center gap-1 max-w-[220px]">
              <span className="truncate">{user.email}</span> <span className="text-cyan whitespace-nowrap">• {user.plan}</span>
            </div>
